...
async function getCalendar(ressource, firstDate, lastDate) {
    ...
    if (URL == false) {
        console.error("[-] Error: ressource not found");
        return null;
    }

    if (URL == true) {
        console.info("[+] Downloading calendar for all ressources");
        for (var key in RESSOURCE) {
            if (key != "all") {
                URL = getRessource(key);
                console.info("[+] Downloading calendar for " + key);
    ...
    console.info("[+] Downloading calendar from " + firstDate + " to " + lastDate);
